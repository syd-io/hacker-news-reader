...
def validate_input(prompt, choices):  # Utility function to validate user input.
    while True:
        try:
...
def fetch_data(url):  # Fetch data from API
    try:
...
def fetch_article_ids(section_choice):  # Get article IDs of section selected by user
...
# Retrieve articles based on article IDs returned from fetch_article_ids()
        ...
        for article_id in article_ids:
            ...
            try:
                ...
                if article and "title" in article and "url" in article:
                    articles.append([len(articles) + 1, article['title'], article['url']])
                    pbar.update(1)
            except requests.exceptions.RequestException:  # Skip articles with HTTP errors
                pass
...
def display_sections():  # Displays the sections available for browsing.
    print("\nWELCOME TO HACKER NEWS!")
    sections = [[1, "Top Stories"], [2, "New Stories"], [
        3, "Best Stories"], [None, None], [0, "Exit"]]
...
def read_article(section_choice, articles):
    article_choice = validate_input(
        ...
        "\nPlease choose an article to read (1-5) or 0 to return to the previous menu: ", range(0, len(articles) - 1))
    if article_choice == 0:
        main()
        return
...
def read_another_article(section_choice, articles):
    ...
    if choose_new_article == 0:
        sys.exit("END")
    print("\n" + tabulate(articles,
                          headers=["No.", "Articles", "URLs"], tablefmt="presto"))
    read_article(section_choice, articles)
